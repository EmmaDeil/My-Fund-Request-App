    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt === 1) {
          ...
          if (!verified) {
            console.warn(`⚠️ [Request ID: ${requestId}] SMTP verification failed, but continuing anyway`);
          }
        }
        ...
        if (error.message.includes("timeout") || errorType === 'ETIMEDOUT') {
          console.error(`⏰ [Request ID: ${requestId}] Timeout error. This may indicate network issues in Render environment.`);
          ...
          console.error(`🌐 [Request ID: ${requestId}] Connection error. Check network connectivity and DNS resolution.`);
        }
        ...
        console.log(`⏳ [Request ID: ${requestId}] Waiting ${Math.round(delay / 1000)}s before retry...`);
        await new Promise((resolve) => setTimeout(resolve, delay));
        
        // Create fresh transporter for retry to avoid connection issues
        if (attempt < maxRetries) {
          console.log(`🔄 [Request ID: ${requestId}] Creating fresh connection for retry`);
          this.transporter = nodemailer.createTransport({
            service: "gmail",
            auth: {
              user: process.env.EMAIL_USER,
