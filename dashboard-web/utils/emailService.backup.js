...
class EmailService {
  constructor() {
    ...
    this.transporter = nodemailer.createTransport({
      ...
      connectionTimeout: 45000, // Reduced timeout for faster failure detection
      greetingTimeout: 20000,
      socketTimeout: 45000,
      tls: {
        rejectUnauthorized: false,
        ciphers: "SSLv3", // Better Gmail compatibility
      },
  ...
  async sendEmailWithRetry(mailOptions, maxRetries = 3, requestId = "Unknown") {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Add connection verification before sending (for first attempt only)
        if (attempt === 1) {
          console.log(`🔍 [Request ID: ${requestId}] Verifying SMTP connection...`);
          try {
            await this.verifyConnection();
          } catch (verifyError) {
            console.warn(`⚠️ [Request ID: ${requestId}] SMTP verification failed, but attempting to send anyway:`, verifyError.message);
          }
        }
        ...
        if (error.code === 'ETIMEDOUT' || error.code === 'ECONNECTION') {
          console.error(`🌐 [Request ID: ${requestId}] Network timeout/connection error. Check SMTP settings and network connectivity.`);
        } else if (error.code === 'EAUTH') {
          console.error(`🔐 [Request ID: ${requestId}] Authentication failed. Check EMAIL_USER and EMAIL_PASS credentials.`);
        }
        ...
        console.log(`⏳ [Request ID: ${requestId}] Waiting ${delay / 1000}s before retry...`);
